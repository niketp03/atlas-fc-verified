...
/-- The improved upper bound $N / (\log N)^2$, one logarithmic factor below
`bestUpper`. -/
noncomputable def improvedUpper (N : ℕ) : ℝ := (N : ℝ) / (Real.log N) ^ 2
...
def repeatPartition25 {B : ℕ} (q : ℕ) (hB : 0 < B)
...
lemma repeatPartition25_card_le {B : ℕ} (q : ℕ) (hB : 0 < B)
...
lemma repeatPartition25_sum_le {B : ℕ} (q : ℕ) (hB : 0 < B)
    (P : Finpartition (Icc 1 B)) :
...
lemma restrict_partition25 {s t : Finset ℕ} (P : Finpartition s) (ht : t ⊆ s) :
    ∃ Q : Finpartition t,
      #Q.parts ≤ #P.parts ∧
...
def scale25 (t : ℕ) : ℕ := 2 ^ (4 * (2 ^ t) ^ 2)



def level25 (N : ℕ) : ℕ := Nat.log 4 (Nat.log 2 N) - 1
...
lemma level25_good (N : ℕ) (hL : 4 ^ 6 ≤ Nat.log 2 N) :
...
noncomputable def fallbackPartition25 (N : ℕ) : Finpartition (Icc 1 N) := by
  let s : Setoid ℕ := ⊤
  letI : DecidableRel s.r := Classical.decRel _
...
$k(N) = O(N/(\log N)^2)$ for which Property25 fails for every large $N$.

This is one logarithmic factor below `bestUpper` $= N/\log N$, and unlike a bare
`=o[atTop] bestUpper` statement it names the rate that the construction achieves.
-/
@[category research solved, AMS 5 11]
theorem green_25.variants.upper_log_sq :
    ∃ k : ℕ → ℕ,
  ...
  · filter_upwards [eventually_ge_atTop (2 ^ (4 ^ 6))] with N hN
    ...
    constructor
    · change 1 ≤ #(selectedPartition25 N).parts
      exact ((selectedPartition25 N).parts_nonempty (by simp [hN1])).card_pos
    · change #(selectedPartition25 N).parts ≤ N
      simpa [Nat.card_Icc] using (selectedPartition25 N).card_parts_le_card
  · rw [Asymptotics.isBigO_iff]
    ...
    set t := level25 N with htdef
    set B := scale25 t with hBdef
    set D := 2 ^ (4 + 4 * t) with hDdef
    have hL : 4 ^ 6 ≤ L := Nat.le_log_of_pow_le (by omega) hN
    have hg : 5 ≤ t ∧ B ≤ N := by
      simpa [hLdef, htdef, hBdef] using level25_good N (by simpa [hLdef] using hL)
    have hspec := selectedPartition25_spec N (by simpa [htdef, hBdef] using hg)
    have htpow : 4 + 4 * t ≤ 2 ^ t := linear_le_pow25 t hg.1
    have hd : 4 + 4 * t ≤ 4 * (2 ^ t) ^ 2 := by
      have hp : 1 ≤ 2 ^ t := one_le_pow₀ (by omega)
      nlinarith
    have hcandD : candidate25 B * D = B := by
      rw [show candidate25 B = 2 ^ (4 * (2 ^ t) ^ 2 - (4 + 4 * t)) by
        simpa [hBdef, scale25, htdef] using candidate25_special t hg.1]
      simp only [hDdef, ← pow_add, Nat.sub_add_cancel hd, hBdef, scale25]
    have hqB : (N / B + 1) * B ≤ 2 * N := by
      calc
        (N / B + 1) * B ≤ N + B := by
          rw [Nat.add_mul]
    ...
    have hansD : ans N * D ≤ 2 * N := by
      calc
        ...
        _ = (N / B + 1) * B := by rw [mul_assoc, hcandD]
        _ ≤ 2 * N := hqB
    have hu : 6 ≤ Nat.log 4 L := Nat.le_log_of_pow_le (by omega) hL
    have htu : t + 1 = Nat.log 4 L := by
      rw [htdef, level25, ← hLdef]
      omega
    have hDE : D = (4 ^ (t + 1)) ^ 2 := by
      simp only [hDdef, show (4 : ℕ) = 2 ^ 2 by norm_num, ← pow_mul, ← pow_add]
      congr 1
      omega
    have hLupper : L < 4 * 4 ^ (t + 1) := by
      have hh := Nat.lt_pow_succ_log_self (by omega : 1 < 4) L
    ...
    have hlogle : Real.log (N : ℝ) ≤ 2 * (L : ℝ) := by
      have hpow := Nat.lt_pow_succ_log_self (by omega : 1 < 2) N
      have hcast : (N : ℝ) ≤ (2 : ℝ) ^ (L + 1) := by
        exact_mod_cast (Nat.le_of_lt (by simpa [hLdef] using hpow))
      have hh := Real.strictMonoOn_log.monotoneOn
        (show (N : ℝ) ∈ Set.Ioi 0 by
          change (0 : ℝ) < N
          exact_mod_cast (Nat.zero_lt_of_lt hN1))
      ...
      calc
        ...
        _ ≤ ((L + 1 : ℕ) : ℝ) * 1 :=
          mul_le_mul_of_nonneg_left hlog2 (by positivity)
        _ ≤ 2 * (L : ℝ) := by push_cast; nlinarith
    have hansreal : (ans N : ℝ) * (L : ℝ) ^ 2 ≤ 32 * (N : ℝ) := by exact_mod_cast hansL
    have hansnn : (0 : ℝ) ≤ (ans N : ℝ) := by positivity
    have hmain : (ans N : ℝ) * Real.log (N : ℝ) ^ 2 ≤ 128 * (N : ℝ) := by
      nlinarith [mul_le_mul_of_nonneg_left
        (mul_self_le_mul_self hlogpos.le hlogle) hansnn]
    have hsqpos : (0 : ℝ) < Real.log (N : ℝ) ^ 2 := by positivity
    rw [Real.norm_eq_abs, abs_of_nonneg hansnn, improvedUpper, Real.norm_eq_abs,
      abs_of_nonneg (by positivity : (0:ℝ) ≤ (N : ℝ) / Real.log (N : ℝ) ^ 2)]
    calc
      (ans N : ℝ) = ((ans N : ℝ) * Real.log (N : ℝ) ^ 2) / Real.log (N : ℝ) ^ 2 := by
        field_simp
      _ ≤ (128 * (N : ℝ)) / Real.log (N : ℝ) ^ 2 := by
        gcongr
      _ = 128 * ((N : ℝ) / Real.log (N : ℝ) ^ 2) := by ring
  · filter_upwards [eventually_ge_atTop (2 ^ (4 ^ 6))] with N hN
    have hL : 4 ^ 6 ≤ Nat.log 2 N := Nat.le_log_of_pow_le (by omega) hN
    have hg := level25_good N hL
    intro hprop
