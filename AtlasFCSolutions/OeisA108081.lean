...
**Problem:** `OeisA108081.count_words_in_x_is_a_shifted` in
[`FormalConjectures/OEIS/108081.lean`](https://github.com/google-deepmind/formal-conjectures/blob/990d209a1ffbe75e59e11267242675931f524824/FormalConjectures/OEIS/108081.lean).

**Solution:** `AtlasFCSolutions.OeisA108081.count_words_in_x_is_a_shifted` at the bottom of this file.

The problem file is imported, and every definition the statement mentions is formal-conjectures'
own (reached through `open OeisA108081`); this file declares no copy of any of them. The final
theorem has the same name as the formal-conjectures theorem and repeats its statement.

The proof is from `Atlas/FC/OeisA108081_count_words_in_x_is_a_shifted.lean` in
...
Mathlib drift; see `NOTES.md`.
-/

-- formal-conjectures' definitions. Opened before `namespace` so that the name refers to
-- formal-conjectures' namespace and not to `AtlasFCSolutions.OeisA108081`.
open OeisA108081

namespace AtlasFCSolutions.OeisA108081
...
lemma XWord.toXInsert {w : Word} (hw : XWord w) : XInsert w := by
  ...
  | @step_right u v hu hv ihu ihv =>
      ...
      exact h₁.trans h₂
alias _root_.OeisA108081.XWord.toXInsert := XWord.toXInsert
      ...
      · intro p s z heq
        ...
        · cases q with
          ...
          | cons b q =>
              ...
              simpa [List.append_assoc] using hnew
alias _root_.OeisA108081.XWord.expand_pair := XWord.expand_pair
...
alias _root_.OeisA108081.XWord.expand := XWord.expand
...
alias _root_.OeisA108081.XWord.exists_expand_predecessor := XWord.exists_expand_predecessor

lemma XWord.prepend_neg_one {w : Word} (hw : XWord w) : XWord ((-1 : ℤ) :: w) := by
  simpa [l] using XWord.step_left XWord.base hw
alias _root_.OeisA108081.XWord.prepend_neg_one := XWord.prepend_neg_one

lemma XWord.append_one {w : Word} (hw : XWord w) : XWord (w ++ [(1 : ℤ)]) := by
  simpa [r] using XWord.step_right hw XWord.base
alias _root_.OeisA108081.XWord.append_one := XWord.append_one
...
alias _root_.OeisA108081.XWord.zero_mem := XWord.zero_mem
...
alias _root_.OeisA108081.XWord.length_pos := XWord.length_pos
...
lemma XWord.value_bounds {w : Word} (hw : XWord w) {z : ℤ} (hz : z ∈ w) :
  ...
  | @step_right u v hu hv ihu ihv =>
      ...
      · have hzv : z - 1 ∈ v := mem_r_iff.mp hz
        ...
        have hulen := XWord.length_pos hu
        omega
alias _root_.OeisA108081.XWord.value_bounds := XWord.value_bounds
...
alias _root_.OeisA108081.XWord.endpoints := XWord.endpoints
...
alias _root_.OeisA108081.XWord.coord_endpoints := XWord.coord_endpoints
...
lemma XWord.length_one {w : Word} (hw : XWord w) (hlen : w.length = 1) : w = [0] := by
  ...
  | @step_right u v hu hv ihu ihv =>
      ...
      omega
alias _root_.OeisA108081.XWord.length_one := XWord.length_one
...
lemma XWord.length_two {w : Word} (hw : XWord w) (hlen : w.length = 2) :
  ...
  | @step_right u v hu hv ihu ihv =>
      ...
      rw [hu.length_one hulu, hv.length_one hvlu]
      simp [r]
alias _root_.OeisA108081.XWord.length_two := XWord.length_two
...
end AtlasFCSolutions.OeisA108081

#print axioms AtlasFCSolutions.OeisA108081.count_words_in_x_is_a_shifted
