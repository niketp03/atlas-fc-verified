...
/-!
# Erdős Problem 138 — `W(k) / 2^k → ∞`

**Problem:** `Erdos138.erdos_138.variants.dvd_two_pow` in
[`FormalConjectures/ErdosProblems/138.lean`](https://github.com/google-deepmind/formal-conjectures/blob/990d209a1ffbe75e59e11267242675931f524824/FormalConjectures/ErdosProblems/138.lean).

**Solution:** `AtlasFCSolutions.Erdos138.erdos_138.variants.dvd_two_pow` at the bottom of this file.

The problem file is imported, and every definition the statement mentions is formal-conjectures'
own (reached through `open Erdos138`); this file declares no copy of any of them. The final
theorem has the same name as the formal-conjectures theorem and repeats its statement.

The proof is from `Atlas/FC/Erdos138_dvd_two_pow_solution.lean` in
...
-- formal-conjectures' namespace and not to `AtlasFCSolutions.Erdos138`.
open Erdos138

namespace AtlasFCSolutions.Erdos138
...
theorem W_div_two_pow_tendsto_atTop :
    erdos_138_variants_dvd_two_pow_statement := by
...
/-- **Erdős 138, `dvd_two_pow` variant.** Statement verbatim from formal-conjectures, with the
answer filled in: `W(k) / 2^k → ∞` does hold. -/
theorem erdos_138.variants.dvd_two_pow :
    answer(True) ↔ atTop.Tendsto (fun k => ((W k : ℚ)/ (2 ^ k))) atTop :=
  ⟨fun _ => W_div_two_pow_tendsto_atTop, fun _ => trivial⟩

end AtlasFCSolutions.Erdos138

#print axioms AtlasFCSolutions.Erdos138.erdos_138.variants.dvd_two_pow
