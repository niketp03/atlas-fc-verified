...
/-!
# Open Quantum Problem 35 — an AME(9, 10) state exists

**Problem:** `OpenQuantumProblem35.ame_9_10_open` in
[`FormalConjectures/OpenQuantumProblems/35.lean`](https://github.com/google-deepmind/formal-conjectures/blob/990d209a1ffbe75e59e11267242675931f524824/FormalConjectures/OpenQuantumProblems/35.lean).

**Solution:** `AtlasFCSolutions.Oqp35.ame_9_10_open` at the bottom of this file.

The problem file is imported, and every definition the statement mentions is formal-conjectures'
own (reached through `open OpenQuantumProblem35`); this file declares no copy of any of them. The final
theorem has the same name as the formal-conjectures theorem and repeats its statement.

The proof is from `Atlas/FC/OpenQuantumProblem35_ame_9_10_open_solution.lean` in
...
-- formal-conjectures' namespace and not to `AtlasFCSolutions.OpenQuantumProblem35`.
open OpenQuantumProblem35

namespace AtlasFCSolutions.Oqp35
...
lemma graphPhase_add {n p : ℕ} (G : Fin n → Fin n → ZMod p)
  ...
  have key : ∀ i j : Fin n, G i j * (x i * d j + d i * x j)
      = x i * G i j * d j + d i * G i j * x j := by
    intro i j; ring
...
/-- **Open Quantum Problem 35, AME(9, 10).** Statement verbatim from formal-conjectures, with
the answer filled in: an AME(9, 10) state does exist. -/
theorem ame_9_10_open :
    answer(True) ↔ ExistsAME 9 10 :=
  ⟨fun _ => existsAME_9_10, fun _ => trivial⟩

end AtlasFCSolutions.Oqp35

#print axioms AtlasFCSolutions.Oqp35.ame_9_10_open
